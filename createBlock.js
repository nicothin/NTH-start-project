...
if (bemBlockName) {
	...
	// в общем виде bemblockName: blockName+[__elementName]+[_modName]+[_modValue]
// разбираем bemblockName
// из конфига получаем разделители, по умолчанию '--' 
let reg=/(?=__)|(?=--)/;
if(bemModSeparator=='_')  {reg=/(?<!_)(?=_)/};
let tempBlockName=[];
tempBlockName=bemBlockName.split(reg);

// путь формируется как blockName/[__elementName]/[_modName]/[_modValue]
let blockPath="";
const blockFileName=tempBlockName[tempBlockName.length-1];
tempBlockName.forEach(function(elem){
	blockPath = blockPath+elem+"/";
});
  const dirPath = `${dir.blocks}${blockPath}`; // полный путь к создаваемой папке блока
  mkdirp(dirPath, (err) => {                    // создаем
    ...
    // Нет ошибки, поехали!
      ...
      extensions.forEach((extension) => {
        const filePath = `${dirPath + blockFileName}.${extension}`; // полный путь к создаваемому файлу
        let fileContent = '';                                   // будущий контент файла
        let fileCreateMsg = '';                                 // будущее сообщение в консоли при создании файла

        if (extension === 'scss') {
          fileContent = `// В этом файле должны быть стили для БЭМ-блока ${bemBlockName}, его элементов,\n// модификаторов, псевдоселекторов, псевдоэлементов, @media-условий...\n// Очередность: http://nicothin.github.io/idiomatic-pre-CSS/#priority\n\n.${bemBlockName} {\n\n  $block-name:                &; // #{$block-name}__element\n}\n`;
          // fileCreateMsg = '';
        }
